...
use remap_protocol::{Rec, ClientEvent, ServerEvent};
...
impl Canvas {
    ...
    pub fn resize(&mut self, width: u32, height: u32) -> Result<(u32,u32)> {
        ...
        // target ~60 FPS; minifb scales the window—framebuffer stays self.width x self.height
    ...
    /// Draw a rectangle into the backbuffer, **clipped** to the framebuffer bounds.
    pub fn draw(&mut self, rec: &Rec) -> Result<()> {
        ...
        // Incoming rect (u16 -> i32 for math)
        let rx = rec.x as i32;
        let ry = rec.y as i32;
        ...
        // Source (rec.bytes) is tightly packed BGRA for a rec.width x rec.height rect
        let src_stride = rec.width as usize * 4;

        // Offset within source if the rect was clipped
        let src_x_off = (x0 - rx).max(0) as usize;
        let src_y_off = (y0 - ry).max(0) as usize;

        let dst_stride = self.width as usize;

        // Copy row by row, converting BGRA -> 0x00RRGGBB (alpha ignored)
        for row in 0..ch {
    ...
    pub fn update(&mut self) -> Result<()> {
        if self.need_update {
            ...
            // process window events without blitting
            self.window.update();
        }
        Ok(())
    }
    ...
    pub fn handle_input(&mut self) -> Result<()> {
        if let Some((xf, yf)) = self.window.get_mouse_pos(MouseMode::Discard) {
            ...
            // send motion on actual move (buttons preserved)
            if self.last_mouse.map(|p| p != (x, y)).unwrap_or(true) {
                self.client_tx
                    .send(ClientEvent::PointerEvent { buttons: self.buttons, x, y })?;
            ...
            // Scroll: send as many pulses as needed for large wheel deltas
            if let Some((_sx, sy)) = self.window.get_scroll_wheel() {
                let mut pulses = sy;
                while pulses >= 1.0 {
                    self.client_tx
                        .send(ClientEvent::PointerEvent { buttons: BTN_WHEEL_UP, x, y })?;
                    pulses -= 1.0;
                }
                while pulses <= -1.0 {
                    self.client_tx
                        .send(ClientEvent::PointerEvent { buttons: BTN_WHEEL_DOWN, x, y })?;
                    pulses += 1.0;
                }
                // restore current buttons state
                if sy != 0.0 {
                    self.client_tx
                        .send(ClientEvent::PointerEvent { buttons: self.buttons, x, y })?;
                }
        ...
        self.window
            ...
            .for_each(|key| {
                let _ = self
                    .client_tx
                    .send(ClientEvent::KeyEvent { down: true, key: *key as u8 });
            });
        self.window.get_keys_released().iter().for_each(|key| {
            let _ = self
                .client_tx
                .send(ClientEvent::KeyEvent { down: false, key: *key as u8 });
        });
    ...
    pub fn handle_server_events(&mut self) -> Result<()> {
        ...
        if any {
            ...
            // Pull-based flow control: request the next incremental frame now
